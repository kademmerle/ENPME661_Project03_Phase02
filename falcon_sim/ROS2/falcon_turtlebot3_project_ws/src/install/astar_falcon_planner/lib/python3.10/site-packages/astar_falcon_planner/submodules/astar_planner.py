...
"""
def move_set(node, u_l, u_r, buffer_set, t_curve=2, r=3.3, L=28.7):
    t    = 0
    cost = 0
    dt   = t_curve / 10
...
def plan_path(start,
    ...
    for i in range(1, len(solution)):
        ...
        dx      = solution[i][0][0] - solution[i-1][0][0]
        dy      = (solution[i][0][1]) - (solution[i-1][0][1])
        d_theta = (solution[i][0][2] - solution[i-1][0][2])/180*math.pi
