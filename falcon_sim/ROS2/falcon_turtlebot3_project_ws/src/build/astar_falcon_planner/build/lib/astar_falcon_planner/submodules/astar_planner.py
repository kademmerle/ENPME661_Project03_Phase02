...
def A_Star(start_node, goal_node, OL, parent, V, C2C, costsum, step, RPM1, RPM2, 
           t_curve, goal_weight,  pxarray, pallet, screen, goal_threshold):
    ...
    while OL:
        ...
        if(euclidean_distance(fixed_node, goal_node) <= goal_threshold ):
            ...
            for action in actions:
                child_node, child_cost = move_set(fixed_node, action[0], action[1], t_curve)
...
def plan_path(start,
    ...
    # Screen dimensions
    rows, cols = (int(540+clearance), 300)
    ...
    DrawBoard(rows, cols, pxarray, pallet, C2C, clearance, robot_radius, screen)
    ...
    while running:
        ...
        alg_state, solution = A_Star(start_node, end, OL, parent, V, C2C, costsum, 
                                     step, rpm1, rpm2, delta_time, goal_weight, pxarray, pallet, screen, goal_threshold)
    ...
    for i in range(1, len(solution)):
        dx = solution[i][0] - solution[i-1][0]
        dy = (solution[i][1]) - (solution[i-1][1])
        d_theta = (solution[i][2] - solution[i-1][2])/180*math.pi
        path.append([dx, dy, d_theta])
    ...
    pygame.draw.circle(screen, pygame.Color(pallet["red"]), (int(end[0]), end[1]), radius=5.0, width=1) # Goal node
